...
// -------------------------------------------Old Way
// func ChangeBackgroundBing(imgItem config.Image) error {
// 	background := ""
// 	wppArray, wppErr := extractBingImageURLs(imgItem)
// 	if wppErr != nil {
// 		fmt.Println("Error: getting web urls on ", imgItem.Location, " for ", imgItem.Operation, " on ", wppErr.Error())
...
// 	background = downloadBingPic(pic, imgItem.Operation+"Choice", wppFolder, imgItem)
// 	if background != "" {
...
// func downloadBingPic(url string, fileName string, folder string, img config.Image) string {
// 	//flickr issue
// 	urlFiltered := ""
// 	ext := ""
// 	urlFiltered1 := strings.Replace(url, "jpg_sm", "jpg", -1)
// 	urlFiltered = strings.Replace(urlFiltered1, "jpg_mb", "jpg", -1)

// 	//For bing the jpg_sm and jpg_mb are small versions for thumb nail and medium size.
// 	//Change that to jpg for full size
...
// 	if ext == "" {
// 		ext = strings.Replace(strings.Split(parts[len(parts)-1], "?")[0], "_sm", "", -1)
// 		ext = strings.Replace(ext, "_mb", "", -1)
// 	}
